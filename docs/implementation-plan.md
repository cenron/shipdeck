...
- Current direction: complete Task 4 by proving the deploy service/engine path through a real Compose-backed Docker runtime adapter, then clean up temporary smoke wiring.
- Current slice: Docker Compose adapter smoke path. `deploy.Service` is wired into `App`, `internal/adapters/docker` exists, `examples/compose-test.yml` is available for local `docker compose` testing, and `StartProject`/`StopProject`/`DeployRevision` now invoke Compose through the adapter. `DeployRevision` trims the requested revision, rejects an empty revision, and maps the revision into `SHIPDECK_TEST_IMAGE` for the smoke fixture.
- Success signal: adapter tests prove `up -d`, `down`, and deploy-revision command/env shape; app-level smoke test records start/stop/redeploy runtime actions and confirms `nginx:1.28-alpine` is passed as the target revision; the overall test suite passes without requiring real Docker.
- Next pickup point: add or strengthen service-level tests for `StartProject`, `StopProject`, `RedeployProject`, and `RollbackProject` through the service boundary. After that, move the hard-coded Compose fixture values (`examples/compose-test.yml`, project name normalization, bind/port/container/image env values) toward a project/config boundary instead of keeping them in the adapter.
...
Status: in progress. Engine domain contracts and behavior are implemented and covered by tests against a fake runtime adapter, including validation guards and rollback success/failure branches. The deploy service is wired through app composition and delegates into the engine. A first Docker adapter scaffold exists at `internal/adapters/docker`, and `examples/compose-test.yml` is the current local Compose smoke fixture. `StartProject`, `StopProject`, and `DeployRevision` now run Compose commands through a shared command executor, with adapter tests covering generated `up -d`/`down` commands and smoke environment values. `DeployRevision` uses the requested revision as the fixture image value, trims whitespace, and rejects an empty revision. App smoke tests now record the runtime action sequence and target revision. Overall tests use a fake Docker executable so normal verification does not require real Docker, and `go test ./...` passed after the redeploy smoke update. Next cleanup is to add service-level tests for start/stop/redeploy/rollback behavior through the service boundary, then move Compose file/project metadata onto the project/config boundary instead of hard-coding fixture paths and env values in the adapter.
