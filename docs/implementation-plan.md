...
- Current direction: start Task 5 by adding registry/source monitoring and update-checking behavior on top of the completed deploy runtime foundation.
- Current slice: prove a project with watched tags can detect a newer registry digest, record that update state, and expose it as an available update without auto-applying it by default.
- Success signal: source/checker tests cover digest comparison, watched-tag OR semantics, update-state persistence through the existing project repository, and no deploy action unless auto-update is explicitly enabled.
- Next pickup point: design the smallest registry source interface and update checker service boundary before adding real registry client behavior. Keep the first tests fake-source driven so normal verification does not require network access.
...
Decision (2026-08-03): production Compose files are imported from the operator and copied into Shipdeck-owned project storage. Shipdeck does not mutate Compose files for the MVP; the Docker adapter only executes a resolved Compose runtime spec. Managed service templates such as Postgres are a post-MVP project-composition workflow, not part of deploy execution.

Status: complete. Engine domain contracts and behavior are implemented and covered by tests against a fake runtime adapter, including validation guards and rollback success/failure branches. The deploy service is wired through app composition and delegates into the engine, with service-level tests covering start, stop, redeploy, rollback, and error propagation through the service boundary. The Docker adapter consumes a resolved `ComposeSpec` from a `ComposeSpecResolver`; `SmokeComposeResolver` temporarily owns the local `examples/compose-test.yml` fixture values while the client only executes Compose commands. `StartProject`, `StopProject`, and `DeployRevision` run Compose through a shared command executor, with adapter tests covering generated `up -d`/`down` commands, deploy-revision command/env shape, and constructor invariants. Normal `App.Run` no longer performs temporary smoke deploy actions. Overall tests use a fake Docker executable so normal verification does not require real Docker, and `go test ./...` passes.
